...
- [ ] 0.1 Confirm OS-1, OS-2 and OS-3 are archived under `openspec/changes/archive/`
...
- [ ] 0.2 Confirm DEC-052 is present and `accepted` in `docs/DECISIONS.md`, and that
      DEC-053 is unused.
...
- [ ] 0.3 Record the baseline: `uv run pytest tests/unit -q` passes, and note the
      pass/xfail counts.
...
- [ ] 1.1 Add **DEC-053**, status `accepted`: pre-generation and post-generation
      metadata lifecycle.
...
- [ ] 1.2 Record DEC-053's three consequences explicitly: TTFT is measured from the
...
- [ ] 1.3 Do not reopen DEC-047, do not rewrite DEC-049, DEC-050, DEC-051 or DEC-052.
...
- [ ] 2.1 Add `ResponseWarning` with `type: Literal["substituted", "degraded"]`,
...
- [ ] 2.2 Add `ResolvedRequest` mirroring `ChatCompletionRequest` minus `messages`,
      ...
      **Why:** the derivability rule makes the block's contents derivable rather than
      ...
      `seed`.
- [ ] 2.3 Append `strict: bool = False` to `ChatCompletionRequest`, **last**.
...
- [ ] 2.4 Add `resolved: ResolvedRequest | None = None` and
      ...
      **After:** 2.2. **Check:** existing `ChatCompletionResponse(...)` call sites
      compile unchanged.
- [ ] 2.5 Do **not** touch `ChatStreamChunk`.
...
- [ ] 3.1 Declare `count_prompt_tokens(self, request) -> int | None`, **non-abstract**,
...
- [ ] 3.2 Do **not** declare `kv_capacity_tokens`.
      **Why:** DEC-047 §3 marks it provisional and forbids freezing it; Phase B4 may
      delete it. **After:** 3.1. **Check:** C9 — `grep -n kv_capacity_tokens
      src/inference_x/engines/base.py` returns nothing.
- [ ] 3.3 Update the class docstring's contract list to name the new declared method.
...
- [ ] 4.1 Add `StrictModeViolationError(ValueError)` with a docstring stating it maps
...
- [ ] 4.2 Widen `AdmissionResult` with `warnings: tuple[ResponseWarning, ...] = ()`.
...
- [ ] 4.3 Replace `_estimate_prompt_tokens`'s `getattr` probe with a direct
      ...
      **Check:** `_CHARS_PER_TOKEN_FALLBACK` is still used; the count is never a
      reported figure.
- [ ] 4.4 Narrow `admit()`'s `engine` parameter from `Any` to `BaseEngine`.
...
- [ ] 4.5 Emit `degraded` / `sequence_gate_skipped` when `_effective_max_num_seqs`
...
- [ ] 4.6 Emit `substituted` / `max_tokens_clamped_to_context` and `substituted` /
...
- [ ] 4.7 Write a structured log record for every warning emitted, at the site that
      emits it.
...
- [ ] 4.8 Update the module docstring: gates 1–3 no longer discover
...
- [ ] 4.9 Do **not** change any admission *decision*.
...
- [ ] 5.1 In `complete()`, build `ResolvedRequest` from `effective_request` and attach
...
- [ ] 5.2 In `stream_response()`, emit exactly one pre-generation event via the
...
- [ ] 5.3 Build `ResolvedRequest` from `effective_request` in both paths, never from
      `request`.
...
- [ ] 5.4 Emit nothing after the usage event, on any path.
...
- [ ] 5.5 Update `stream_response()`'s docstring event list to include the
...
- [ ] 6.1 Anchor TTFT to the first event carrying non-empty `delta.content` instead of
...
- [ ] 6.2 Do **not** add any timing field, metric, or exporter.
      **Why:** Phase B2/B3. This task preserves an existing measurement's meaning; it
      ...
      unmodified.
- [ ] 6.3 Leave `_extract_sse_usage` unchanged.
...
- [ ] 7.1 Update `count_prompt_tokens`'s docstring (lines 469–477): it is now a
...
- [ ] 7.2 Do **not** change the method's internal chars/4 fallback, its signature, or
      `kv_capacity_tokens`.
...
- [ ] 8.1 `tests/unit/test_chat_service.py` — extend the OS-2 ordering tests to assert
...
- [ ] 8.2 `tests/unit/test_chat_service.py` — assert the pre-generation event precedes
...
- [ ] 8.3 `tests/unit/test_chat_service.py` — DEC-052 §3: for a request accepted under
...
- [ ] 8.4 `tests/unit/test_admission.py` — make `_FakeEngine` subclass `BaseEngine`.
      **Why:** 4.4 narrows the parameter type; a plain class would make the annotation
      ...
      otherwise.
- [ ] 8.5 `tests/unit/test_admission.py` — DEC-052 §2: assert the set of conditions
...
- [ ] 8.6 `tests/unit/test_admission.py` — for each `degraded` condition, assert the
...
- [ ] 8.7 `tests/unit/test_admission.py` — assert an unclamped, capability-complete
...
- [ ] 8.8 `tests/unit/test_schemas.py` — derivability test: `ResolvedRequest`'s field
      ...
      **Why:** enforces the rule in CI rather than in review, so a future field forces
      ...
      gains a field.
- [ ] 8.9 `tests/unit/test_schemas.py` — assert `strict` defaults to `false`, `resolved`
...
- [ ] 8.10 `tests/unit/test_routes.py` — assert `strict: true` on a clamping request
...
- [ ] 8.11 `tests/unit/test_observability.py` — assert TTFT is measured from the first
...
- [ ] 8.12 `tests/unit/test_observability.py` — assert `_extract_sse_usage` returns
      `None` for the pre-generation event.
      **Why:** C4 — no token figure may originate from it. **After:** 6.3.
- [ ] 8.13 Consumer compatibility — assert `benchmarks/runner.py` and
...
- [ ] 9.1 **No Phase B work.** `engines/driver.py`, `observability/exporters.py`,
      `utils/vllm_pool_config.py` and `api/deps.py` are unmodified.
- [ ] 9.2 **No Phase C work.** No replay handle, no trace id, no batch-invariance
      claim, no determinism claim anywhere in code or docs.
- [ ] 9.3 **No AsyncLLM migration.** `EngineDriver` keeps its locking, dead-flag
      semantics (DEC-043) and restart policy.
- [ ] 9.4 **No timings.** No timing field on any schema, no new metric, no exporter.
      Task 6.1 preserves an existing measurement and adds nothing.
- [ ] 9.5 **No `inference_x/execution/`**, no backend-neutral DTO package, no second
      type system.
- [ ] 9.6 **No second backend.** `ModelEntry.engine` remains `Literal["vllm"]`;
      `vllm` remains a required dependency, not an optional extra.
- [ ] 9.7 **No `kv_capacity_tokens` declaration.** Both `getattr` sites
      (`routing/admission.py:212`, `api/routes/metrics.py:43`) are byte-identical.
- [ ] 9.8 **No registry factory.** `engines/registry.py` stays at 0 bytes; the
      ARCHITECTURE.md gap remains a tracked open item and the doc is not weakened to
      match the code.
- [ ] 9.9 **No scheduler or batching change.** No change to `max_num_seqs`,
      `max_num_batched_tokens`, block size, prefix caching, or any dispatch ordering.
- [ ] 9.10 **No `playground/` change.** It is a client. Its two `len(content.split())`
      displays remain a recorded follow-up, not OS-4 work.
- [ ] 9.11 **No `ChatStreamChunk` widening** and no change to
      `BaseEngine.generate_stream`.

## 10. Validation and self-review

- [ ] 10.1 Every Compatibility Invariant C1–C16 holds.
- [ ] 10.2 Only Intentional Behavioral Changes B1–B7 landed.
- [ ] 10.3 The DEC-048 mypy baseline has not grown (C14).
- [ ] 10.4 `uv run ruff check .` and `uv run mypy src/` pass.
- [ ] 10.5 Full unit suite passes; the xfail count is unchanged from 0.3.
- [ ] 10.6 `git diff --stat` touches no file outside `proposal.md`'s Repository impact.
- [ ] 10.7 `openspec validate --strict` passes for this change.
- [ ] 10.8 Do not mark complete if any acceptance criterion in `proposal.md` is unmet.
