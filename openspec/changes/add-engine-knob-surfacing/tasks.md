...
- [ ] 1.1 `config/vram_tiers.yaml`: add `max_num_batched_tokens`,
  `enable_prefix_caching` per tier (6gb: 2048 / false; 12gb: 4096 / true; 24gb:
  8192 / true)
- [ ] 1.2 `utils/vram_tiers.py`: extend `VramTier` dataclass + `from_dict` with the
  two new fields, `.get()`-defaulted for backward compatibility
- [ ] 1.3 `schemas/model.py`: add `ModelEntry.max_num_batched_tokens: Optional[int]`
  override (same `Field(ge=1, description=...)` shape as `max_num_seqs`)

## 2. Knob resolution

- [ ] 2.1 New `apply_tier_knobs(model_config, tier)` in `utils/vllm_pool_config.py`
...
- [ ] 2.2 Unit tests: model override smaller than tier value honored; larger value
  clamped down to tier ceiling; no-tier fallback behaves like today

## 3. Engine wiring

- [ ] 3.1 `VLLMEngine.__init__`: pass `max_num_batched_tokens`,
...
- [ ] 3.2 `api/deps.py`: `_build_engine_pool` resolves the VRAM tier (fail-open with
...
- [ ] 4.1 `routing/admission.py`: new `_InFlightSeqTracker` (per-model in-flight
  request count, thread-safe, same shape as `_KVReservationTracker`)
- [ ] 4.2 `AdmissionController.admit()`/`release()`: increment/decrement the
...
- [ ] 4.3 Unit tests: saturation at `max_num_seqs` rejects both interactive and
...
- [ ] 5.2 Live check: loading `qwen2.5-7b-awq` standalone with the 6gb tier's
...
- [ ] 6.1 Add a DEC entry to `docs/DECISIONS.md` documenting the knob wiring and
